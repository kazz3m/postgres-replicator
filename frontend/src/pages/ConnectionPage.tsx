...
function getConnectionHint(error: string): string | null {
  ...
  if (error.includes('password authentication failed'))
    return 'Hint: Check username and password.'
...
export function ConnectionPage({ onConnected, onBack }: Props) {
  const [srcAdmin, setSrcAdmin] = useState<ConnFields>(emptyFields())
  const [srcRepl, setSrcRepl] = useState<ConnFields>(emptyFields())
  const [dst, setDst] = useState<ConnFields>(emptyFields())
  const [showReplFields, setShowReplFields] = useState(false)
  ...
  const sourceDsn = buildDsn(srcAdmin)
  const sourceReplDsn = showReplFields ? buildDsn(srcRepl) : ''
  const destDsn = buildDsn(dst)
  const canConnect = !!(sourceDsn && destDsn)

  function loadProfile(p: ConnectionProfile) {
    setSrcAdmin(parseDsn(p.source_dsn))
    if (p.source_repl_dsn) {
      setSrcRepl(parseDsn(p.source_repl_dsn))
      setShowReplFields(true)
    } else {
      setSrcRepl(emptyFields())
      setShowReplFields(false)
    }
    setDst(parseDsn(p.dest_dsn))
    setTestResult(null)
    setError('')
  }
      ...
      <div className="w-full max-w-2xl space-y-4">
        ...
        {profiles.length > 0 && (
          <div className="bg-gray-900 border border-gray-700 rounded-lg overflow-hidden">
            <button
              className="w-full flex items-center gap-2 px-4 py-3 text-sm font-semibold text-gray-300 hover:bg-gray-800"
            ...
            >
              ...
              <span className="text-xs bg-blue-900 text-blue-300 rounded-full px-2 py-0.5">{profiles.length}</span>
            </button>
            {profilesOpen && (
                  ...
                  <div key={p.id} className="px-4 py-3 flex items-center gap-3 hover:bg-gray-800/50">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-gray-500 mt-0.5 truncate">
                        ...
                        {p.source_repl_dsn && <span className="ml-2 text-blue-500">[+repl]</span>}
                        {'  '}
                    ...
                    {deleteConfirm === p.id ? (
                      <div className="flex items-center gap-1.5 shrink-0">
                        ...
                        <button onClick={() => handleDeleteProfile(p.id)}
                          className="text-xs text-red-400 border border-red-800 px-2 py-1 rounded">Yes</button>
                        <button onClick={() => setDeleteConfirm(null)}
                          className="text-xs text-gray-400 border border-gray-700 px-2 py-1 rounded">No</button>
                      </div>
                    ) : (
                      <button onClick={() => setDeleteConfirm(p.id)}
                        className="text-gray-600 hover:text-red-400 p-1 rounded hover:bg-red-950/30 shrink-0">
                        <Trash2 size={13} />
                      </button>
                    )}
                  </div>
        ...
        <ConnectionFields
          ...
          hint="Superuser or user with CREATE PUBLICATION privilege. Used for analysis and management queries."
          fields={srcAdmin}
          onChange={setSrcAdmin}
        />

        {/* ── Source replication (toggle) ── */}
        <div>
          <button
            onClick={() => setShowReplFields(s => !s)}
            className="text-xs text-gray-400 hover:text-gray-200 flex items-center gap-1.5 mb-2"
          >
            {showReplFields ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
            {showReplFields ? 'Hide' : 'Add'} separate Replication user (optional)
          </button>
          {showReplFields && (
            <ConnectionFields
              label="Source — Replication User"
              labelColor="text-cyan-400"
              hint="Used in CREATE SUBSCRIPTION CONNECTION clause. Must have REPLICATION attribute and SELECT on published tables."
              fields={srcRepl}
              onChange={setSrcRepl}
            />
          )}
        ...
        <ConnectionFields
          ...
          hint="User with CREATE privilege on the database, or pg_create_subscription role (PG 16+)."
          fields={dst}
          onChange={setDst}
        />
                ...
                {saveFormOpen ? (
                  <div className="flex items-center gap-2">
                    <input type="text" value={profileName} onChange={e => setProfileName(e.target.value)}
                      ...
                      placeholder='Profile name, e.g. "prod to staging"'
                    ...
                    <button onClick={handleSaveProfile} disabled={!profileName.trim()}
                      className="px-3 py-1.5 bg-blue-700 hover:bg-blue-600 disabled:opacity-50 rounded text-sm font-semibold">Save</button>
                    <button onClick={() => { setSaveFormOpen(false); setProfileName('') }}
                      className="px-3 py-1.5 bg-gray-700 hover:bg-gray-600 rounded text-sm">Cancel</button>
                  </div>
                ) : (
                  <button onClick={() => setSaveFormOpen(true)}
        ...
        <div className="flex gap-3">
          <button onClick={handleTest} disabled={loading || !canConnect}
        ...
        <p className="text-xs text-gray-600">Passwords are never logged or sent in plain text after connection.</p>
      </div>
    </div>
  )
}
